...
(defun uab-calculate-free-algebra (algebra-project size-of-algebra
  ...
  "Writes commands to calculate the free algebra of ALGEBRA-PROJECT into
(COMMAND-FILE ALGEBRA-PROJECT)"
...
in NUMBER-OF-GENERATORS generators."
  (let ((origin-project (make-uacalc-project (generate-unique-pathname)))
	(result-project (make-uacalc-project (generate-unique-pathname))))
    (uacalc-write-algebra-to-file algebra origin-project)
    (uab-calculate-free-algebra origin-project (card (base-set-of algebra))
				number-of-generators result-project)
    (uacalc-read-algebra-from-file result-project)))
